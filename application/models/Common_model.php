...
class Common_model extends CI_Model {
	...
	function updateStartChatTime($key,$id,$data,$table){
...
function get_counts($tbl,$where) {
    ...
    public function checkall_review($table, $where,$where1) {
        ...
        $this->db->select('Reviews.*, rating.*, review_reply.reply as reply');

        $this->db->from($table);

        $this->db->join('rating', 'rating.reviewId = Reviews.id');
		$this->db->join('review_reply', 'review_reply.review_id = Reviews.id','left');
	...
	public function checkAvb($id){
		...
		return $data["review"] = $this->db->select('Sun, Mon, Tue, Wed, Thu, Fri, Sat')->from('weekly_availability')->where('astro_id',$id)->get()->result();
