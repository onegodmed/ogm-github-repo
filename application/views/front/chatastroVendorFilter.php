...
.askastro .callbtn .follow.btn-style1 .btnnty i {
    margin-right: 0;
}
...
.askastro .callbtn .follow.btn-style1 .btnnty {
    width: 30px;
    height: 30px;
    border-radius: 50%;
    text-align: center;
...
<!-- Loader ends here -->
<div id="partial">
   <div class="articlemrg chat askastro" id="astlist">
      <?php
         if (!empty(@$astro)) {
		    ...
		    <?php
						...
						if($value->experience)
						{
							$ex=explode(' ',$value->experience);
							$asexperience = $ex[0];
						}
                        ?>			 
      <div class="li astrolist" id="19328" data-price="<?php echo $chat_price; ?>" data-rating='<?php echo number_format($rating1[0]->rating, 2); ?>' data-exp='<?php echo $asexperience; ?>'>
               ...
               <div class="as_profile_text">
                  <ul class="as_profile_text_ul">
                     <?php
                        ...
                        ?>
                     <li class="wrapText" title="<?php echo $value->expertise . ',' . $expertise; ?>"><i class="fa fa-thumbs-up" aria-hidden="true"></i><?php echo $value->expertise . ',' . $expertise; ?></li>
                     <li class="wrapText astroLang" data-category="<?php echo str_replace(",", " ", $value->langknown); ?>" title="<?php echo $value->langknown ?>"> <i class="fa fa-language" aria-hidden="true"></i><?php echo $value->langknown ?></li>
                     <li> <i class="fa fa-briefcase" aria-hidden="true"></i><?php echo $value->experience ?></li>
                     ...
                     <?php }?>
                     <li> <i class="fa fa-inr" aria-hidden="true"></i> <span class="discount_price">Chat: <?php if($actual_chat_price > 0){?><span> <strike> <?php echo $actual_chat_price; ?>/Min</strike></span><?php }?><span> <?php echo $chat_price; ?>/Min</span></span> </li>
                     ...
                     <?php } else { ?> 
                     <li> <i class="fa fa-usd" aria-hidden="true"></i><span class="discount_price">Chat:<span> <?php echo $dollar_chat_price; ?>/Min</span></span> </li>
                  ...
                  <?php if ($queue == '2') { ?>
                      <?php  if($this->session->userdata('id') == ''){ ?>
                      <a class="buy_btn online_queue popup-with-zoom-anim" href="#login-dialog" style="background:#f77403!important;color: #fff!important;font-size:15px;">Busy</a>
                      <?php }else {?>
                      <a data-toggle="modal" data-target="#astroModal" class="buy_btn online_queue" style="background:#f77403!important;color: #fff!important;font-size:15px;">Busy</a>
                      <?php }?>
                  <?php } else { ?>
                    <div class="astroModalPopup"> 
                     ...
                     <input type="hidden" class="astroChatPrice" id="astrochat_<?php echo $dollar_chat_price; ?>" name="astrochat_<?php echo $dollar_chat_price; ?>" value="<?php echo $dollar_chat_price; ?>"/>
                       ...
                       <div class="onlineastro">
                           <div class="online-inner">
                           <a  class="buy_btn online_queue JoinCheck"   style="background:#f77403!important;color: #fff!important;font-size:15px; cursor:pointer;"><?php if(isset($userID)) { if(((isset($userID[0]) &&$userID[0] == $this->session->userdata('id')) || (isset($userID[1]) && $userID[1] == $this->session->userdata('id')))) {?>Queue Joined <?php } else { ?> <span > Join Queue </span>  <?php } }else { ?>  <span > Join Queue </span> <?php }?></a>
                           </div>
                  ...
                  <?php if ($queue == '2') { ?>
                     <?php  if($this->session->userdata('id') == ''){ ?>
                      ...
                      <?php }?>
                     <?php } else { ?>
                     <a  class="buy_btn online_queue JoinCheck popup-with-zoom-anim" href="#login-dialog" style="background:#f77403!important;color: #fff!important;font-size:15px; cursor:pointer;">Join Queue</a>
                  ...
                  <div class="callbtn btnvnd<?php if(($value->next_avail_chat)!='')  { ?> msgcame<?php } ?>">
                     <div class="follow btn-style1">
                       <?php if($this->session->userdata('id') == '' ){?>
                       	 <a class="btnnty popup-with-zoom-anim" href="#login-dialog" alt="Notify">
                            	<i class="fa fa-bell-o profile-icon" aria-hidden="true"></i>
                           	  </a>
...
<script type="text/javascript">
   ...
   $('#promodisc').val(0);
       ...
       $(".astroModalPopup").on('click', function (event) {
           ...
           $('#pickmsg').text('If ' + astroName + ' does not start the chat in Next 2 Minutes , You May Please choose another Astrologer.');
                   ...
                   success: function (data) {
                       ...
                       if(callerId.status == 1){
                        ...
                        // alert("Astro ID" + astroid);
                           ...
                           $.ajax({
                              url: "https://www.onegodmed.com/send",
                              method: "POST",
                              data: {userid: astroid}, 
                            //   type: 'POST',
                            //   dataType: "json",
