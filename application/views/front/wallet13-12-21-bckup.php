...
.discount-image1 span{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#fff;font-weight:bold;}
#packname1000 {
...
<div class="ast_journal_wrapper ast_toppadder70 ast_bottompadder70 walletpage" >
	<div class="container">
		<div class="row">
			...
			<div class="serc_flds">
			   <input type="text" value="" name="" class="pack_amt" placeholder="Enter Amount in INR"/>
               ...
               if (response.status == 1) {
                     $('#mkey').val(response.message['mkey']);
                      $('#hash').val(response.message['hash']);
                       ...
                       setTimeout(function(){ document.getElementById("payuForm").submit(); }, 500);
                }
            }            
...
if ($(window).width() < 768) {
         $('.ast_packages_box1').attr({'data-toggle':'modal','data-target':'#myModalopn'});
         $('#myModalopn').addClass("modal fade");
         $('#myModalopn').attr('role','modal');
    } else {
        $("#myModal1").remove();
         $('.ast_packages_box1').removeAttr({'data-toggle':'modal','data-target':'#myModalopn'});
