...
class User extends REST_Controller {
    ...
    // Test TO DO
    public function forgotPassword_post(){  // in use API
         
            $postdata = file_get_contents("php://input");
            $userData = json_decode($postdata);
            $email = $userData->email;

            $to = "mangeshap@gmail.com";
            $subject = "Forgot password email";
            $txt = "Your password has been reset - New password is 12345 ";
            $headers = "From: mangeshap@gmail.com" . "\r\n" .
            "CC: pmangesh@onegodmed.com";

            $data['password'] = "12345";
            $result = $this->common_model->updateData("email", $email, $data, "author");
            
            mail($to,$subject,$txt,$headers);

            if ($result == true) {         
                $resultData['status'] = true;
                $resultData['message'] = "Your password has been emailed ... ";
                $httpCode = 200;
                mail($to,$subject,$txt,$headers);
                return $this->response($this->json($resultData), $httpCode);                
            } else {
