...
redirect('partner/');
        ...
        $notifData = [
            ...
            'body' => "Customer is waiting for you...!!!!",
            'sound' => "notificationsound.mp3" //notificationsound.wav
        ];
        
        // Create the api body
        $apiBody = [            
            'data' => $notifData,
            'to' => $data[0]->devicetoken, //"cPhV71QERyWed-QXtSQAcZ:APA91bGEB6B-p8c-ETtZ88l4IDMrcaxAIpWoqZrf8lxb_x6A5AxJ9FrwgP5pq2uugiKeElFq05vy8clYWSQj86nxuYyAi4YNL9RgpKklj1IYSw0ugEZZQclNCOSYRX3iFQu8MlHvu_XJ"
            'sound' => "notificationsound.mp3" //notificationsound.wav
        ];
    ...
    // Chat notification FCM integration
    ...
    public function credit(){
        ...
        // Add notification content to a variable for easy reference
        $notifData = [
            'title' => "Hurray! Amount Credited.. !!!.",
            'body' => "Amount for this booking has credited to wallet...!!!!",
        ];
        ...
        curl_close ( $ch );
         ...
         $resultData['status'] = true;
         $resultData['data'] = $result;
         $resultData['message'] = "Hurray! Amount Credited.. !!!.";
         $httpCode = 200; 
...
if(($data==FALSE)) {
    ...
    // Wallet page - revised package list from DB/ backend 
    // Rs 1 and extra amount packages list w/ coupon code
    // Mangesh Prabhavalkar
    // Date - 09-02-2022

    public function wallet_new($id = "") {
        ...
        $this->load->view('front/header', $data);
        if ($this->session->userdata('countryCode') == 'IN') {
            $this->load->view('front/wallet-new', $data);
        } else {
            $this->load->view('front/walletusd', $data);
        }
        $this->load->view('front/footer', $data);
...
"serviceno": "4474225",
 ...
 $agent_otp = $this->input->post('agent_otp');
    ...
    $callerID = $agent_otp;
            ...
            echo json_encode(array("status" => 0, "message" => "failed1"));
        }  
        }   
        }
...
"serviceno": "4474225",
 ...
 $agent_otp = $this->input->post('agent_otp');
  ...
  public function pay() {
        ...
        $_SESSION['pack_amt_dsc'] = $amt_bfr_dsc; // pachake recharge value w/ cashback
    ...
    function success(){
        ...
        // $amt_bfr_dsc = $this->input->post('pack_amt_dsc');
        $amt_bfr_dsc = $_SESSION['pack_amt_dsc'];
        
        // Added for wallet update
        $aWallet = $this->front->getAuthorWalletById($this->session->userdata('id'));
        $wb = $aWallet[0]['wallet_balence'] + $amt_bfr_dsc;

        // echo "WALLET CURRENT ";
        // print_r($aWallet[0]);
        // echo "WALLET TOTAL " . $wb;
        // exit;

        //Update wallet as per current reacharge 
        $arrWallet = array('wallet_balence'=>$wb);
...
public function  packageRecharge(){
    ...
    if($_POST){
        ...
        $cashback = $_POST['pack_amt_real'] - $_POST['pack_amt'];
        ...
        $_SESSION['base_price_wo_gst'] = $_POST['pack_amt'];
