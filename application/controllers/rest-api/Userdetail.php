...
class Userdetail extends REST_Controller {
    ...
    public function credit_post(){
        ...
        $where = array('id' => $uid);
        $query_result = $this->admin->get_details('author', $where);
        if (count($query_result) > 0) {
            $resultData['status'] = true;
            $resultData['data'] = $query_result;
            $resultData['message'] = "Successful";
            $httpCode = 200;
        } else {
            $resultData['status'] = false;
        ...
        'data' => $notifData,
        // "time_to_live" => "600", // Optional
        'to' => $query_result[0]->devicetoken, // Replace 'mytargettopic' with your intended notification audience
        ];
...
$status = $this->admin->_updateRowWhere('order_history',$data1,$where);
    ...
    public function weeklyav_post(){
        ...
        if($userData->userid >0){
            ...
            $data['review'] = $this->common_model->checkAvb($userData->userid);
            if (count($check)>0) {
                $resultData['status'] = true;
                $resultData['data'] = $data['review'];
                $resultData['message'] = "Successfull.";
