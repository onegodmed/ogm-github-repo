    ...
    public function chatsend() {
        ...
        $arData = array(
                ...
                $where2 = array('id' => $this->input->post('order_id'));
                  $data['order_chat'] = $this->admin->get_details("order_history", $where2); 
                  if(($data['order_chat'][0]->start_chatTime == "") || ($data['order_chat'][0]->start_chatTime == NULL)){
                      $data1 = array(
...
echo json_encode(array("status"=>0));
 ...
 public function chatending(){
     ...
     $order_id = $this->input->post('order_id'); // order ID
     $where = array('id' => $order_id);
     $order_detail = $this->admin->get_details('order_history', $where);
     
    //  print_r($order_detail);
    //  echo json_encode(array("status" => $order_id));
    //  die;

    
     if($order_detail[0]->is_Active == "Accept" || $order_detail[0]->is_Active == "Running"){
        $actual_price = $order_detail[0]->actual_price; // main rate e.g. 35   
        $vendor_price = $order_detail[0]->vendor_price; // 20
        $startchatTime = $order_detail[0]->start_chatTime; // once the chat is accepted
        // echo json_encode(array("status" => $startchatTime));
   
        
        
        if($startchatTime != ""){
            $astro_id = $order_detail[0]->astrologer_id; // 3756 - Astro Mang
            $cust_id = $order_detail[0]->customer_id; // 29 - admin
            ...
            $vFmins = ($aMins>5) ? $aMins -1 : $aMins; // if less than & equal 5 then take complete duration - NO ONE MIN FREE  
            $dMins = ($fMins > 5) ? $fMins-1 : $fMins; // if greater than 5 then 1 MIN FREE 
            ...
            $vFsecs = ($aMins * 60) + $aSecs;
                ...
                $timeSpend = "";
                    ...
                    if ($sec > 0)
                        ...
                        // $timeSpend = 20; // comment this line

                    $call_status1 = "Complete";
                    
                    // print_r($timeSpend);
                    // echo json_encode(array("status" => $timeSpend));
                    ...
                    if(count($getQueueData)>0){
                        ...
                        // print_r($UserId);
                        echo json_encode(array("status" => $UserId));
                        ...
                        // print_r($orderID);
                        // echo json_encode(array("status" => $orderID));
                        // die;
                    ...
                    $cust_wallet = $this->admin->_updateRowWhere('author', $data_cust, $where_cust);
                        ...
                        // print_r($astro_wallet);
                        // echo json_encode(array("status" => $astro_wallet));
                        // die;
